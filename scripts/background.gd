...
func playfield_offset_x() -> float:
    return _playfield_offset_x
...
func apply_canvas_size(viewport_size: Vector2) -> void:
    ...
    _playfield_offset_x = PortraitLayout.playfield_offset_x(_canvas_size)
...
func _draw() -> void:
    ...
    var center_x := _playfield_offset_x + PortraitLayout.DESIGN_SIZE.x * 0.5

    draw_rect(Rect2(Vector2.ZERO, _canvas_size), Color("050816"))

    # Soft horizon bands continue across expanded phone and desktop viewports.
    ...
    # Star positions are normalized so resizing never leaves empty canvas.
    for star in stars:
    ...
    # The 720-wide gameplay geometry is centered inside any extra horizontal
    # canvas exposed by stretch/aspect="expand".
    for lane_x in GameBalance.LANE_X:
        var x := _playfield_offset_x + lane_x
    ...
    # Player-zone glow follows the centered gameplay column.
