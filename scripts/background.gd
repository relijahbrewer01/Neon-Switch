...
func _ready() -> void:
    ...
    for i in range(64):
        stars.append({
            "x": rng.randf_range(28.0, GameBalance.VIEWPORT_SIZE.x - 28.0),
            "y": rng.randf_range(0.0, GameBalance.VIEWPORT_SIZE.y),
...
func _draw() -> void:
    var width := GameBalance.VIEWPORT_SIZE.x
    var height := GameBalance.VIEWPORT_SIZE.y
    var center_x := width * 0.5

    draw_rect(Rect2(Vector2.ZERO, GameBalance.VIEWPORT_SIZE), Color("050816"))
